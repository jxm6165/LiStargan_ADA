...
from core.augment import AugmentPipe
...
class Solver(nn.Module):
    ...
    def train(self, loaders):
        ...
        for i in range(args.resume_iter, args.total_iters):
            ...
            real_logits, d_loss, d_losses_latent = compute_d_loss(
                nets, args, augment_pipe, x_real_=x_real, y_org=y_org, y_trg=y_trg, z_trg=z_trg, masks=masks)
            self._reset_grad()
            d_loss.backward()
            ...
            if (i+1) % args.print_every == 0:
                ...
                log += ' Augment_p: %.4f' % augment_pipe.p
                print(log)
...
def compute_d_loss(nets, args, augment_pipe, x_real_, y_org, y_trg, z_trg=None, x_ref=None, masks=None):
    assert (z_trg is None) != (x_ref is None)
    # with real images
    x_real = augment_pipe(x_real_)
    ...
    return real_logits, loss, Munch(real=loss_real.item(),
                       fake=loss_fake.item(),
                       reg=loss_reg.item())
